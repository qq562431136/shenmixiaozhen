...
class Oauth
{
    ...
    {   
        ...
        try {
            ...
            return json(new SuccessMessage('Invalid authorization credentials',401));
        }
    }
    ...
    public static function certification($data = []){
        ...
        if(!$getCacheAccessToken){
            return json(new SuccessMessage('access_token不存在或为空',401));
        }
        if($getCacheAccessToken['client']['appid'] !== $data['appid']){
            return json(new SuccessMessage('appid错误',401));//appid与缓存中的appid不匹配
        }
        return $data;
    }
